...
class Executor {
    ...
    buildFields(fields = "*") {
        ...
        if (Array.isArray(fields)) {
            return fields.map(f => (0, utils_1.quote)(f)).join(', ');
        }
    ...
    async findOne(query, options = {}) {
        ...
        if (sql)
            joinSql += ` WHERE ${sql} `;
        joinSql += ` ${(0, parse_1.parseOrder)(sort)} LIMIT 1`;
    ...
    async count(query) {
        ...
        let joinSql = `SELECT COUNT(*) AS total FROM ${(0, utils_1.quote)(this.table)}`;
        if (sqlWhere)
    ...
    prepareFields(data) {
        ...
        for (const key in data) {
            ...
            const quotedKey = (0, utils_1.quote)(key);
            fields.push(quotedKey);
            if (fieldType === schema_1.DataType.Json && typeof value === "object" && value != null) {
    ...
    async insert(data, opt = {}) {
        ...
        let sql = `INSERT ${ignore ? 'IGNORE' : ''} INTO ${(0, utils_1.quote)(this.table)} (${insertFields.join(', ')}) VALUES (${params.map(() => '?').join(', ')})`;
        // 处理 UPSERT 逻辑
        if (upsert) {
            ...
            const updateSql = updateFields
                .map(f => `${(0, utils_1.quote)(f)} = VALUES(${(0, utils_1.quote)(f)})`)
                .join(', ');
    ...
    async insertMany(data, opt = {}) {
        ...
        const quotedColumns = columns.filter(c => this.schema.fieldsMap.has(c)).map(c => (0, utils_1.quote)(c)).join(', ');
        const results = {
            fieldCount: 0,
            affectedRows: 0,
            insertId: 0,
        ...
        const insert = async () => {
            for (const batchData of batch) {
                ...
                let sql = `INSERT ${ignore ? 'IGNORE' : ''} INTO ${(0, utils_1.quote)(this.table)} (${quotedColumns}) VALUES ${valuesSql}`;
                if (upsert) {
                    const updateFields = Array.isArray(upsert)
                        ? upsert
                        : Object.keys(data).filter(k => this.schema.fieldsMap.has(k));
    ...
    async deleteOne(query) {
        ...
        const { sql: whereSql, params: whereParams } = (0, parse_1.parseQuery)(query);
        const finalSql = `DELETE FROM ${(0, utils_1.quote)(this.table)} WHERE ${whereSql} LIMIT 1`;
    ...
    async deleteMany(query) {
        ...
        const { sql: whereSql, params: whereParams } = (0, parse_1.parseQuery)(query);
        const finalSql = `DELETE FROM ${(0, utils_1.quote)(this.table)} WHERE ${whereSql}`;
        const result = await this.execute(finalSql, whereParams);
        if (typeof this.schema.hooks.afterDelete === "function") {
            return await this.schema.hooks.afterDelete(result);
        }
