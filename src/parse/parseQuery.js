        ...
        for (let key of keys) {
            ...
            if (index_js_1.LogicalMap[key]) {
                ...
                if (key == "$and" || key == "$or") {
                    ...
                    segments.push(`(${arr.join(` ${index_js_1.LogicalMap[key]} `)})`);
                }
                else if (key === '$not') {
            ...
            if (value && typeof value === 'object' && !Array.isArray(value)) {
                ...
                for (const op of keys) {
                    ...
                    if (!index_js_1.QueryOperatorMap[op]) {
                    ...
                    else if (index_js_1.QueryOperatorMap[op]) {
                        if (op == "$in" || op == "$nin") {
                            if (!Array.isArray(val)) {
                            ...
                            segments.push(`${key} ${index_js_1.QueryOperatorMap[op]} (${val.map(() => "?").join(",")})`);
                            params.push(...val);
                        }
                            ...
                            segments.push(`${key} ${index_js_1.QueryOperatorMap[op]} ?`);
                            params.push(val);
                        }
                        else {
                            if (val && typeof val !== "string" && typeof val !== 'number') {
